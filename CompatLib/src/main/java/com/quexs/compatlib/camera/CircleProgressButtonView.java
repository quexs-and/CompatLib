...
public class CircleProgressButtonView extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mProgressAni.setDuration(mTime * 1000L);
    }
            ...
            switch (msg.what) {
                case WHAT_LONG_CLICK:
                    ...
                    if (onLongPressListener != null) {
                        onLongPressListener.onLongClick();
                    }
                    //内外圆动画，内圆缩小，外圆放大
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mEndTime - mStartTime < mLongClickTime) {
                    ...
                    if (mProgressAni != null && mProgressAni.getCurrentPlayTime() / 1000 < mMinTime && !isMaxTime) {
                        if (onLongPressListener != null) {
                            onLongPressListener.onNoMinRecord(mMinTime);
                        }
                        mProgressAni.cancel();
                    } else {
                        //录制完成
                        if (onLongPressListener != null && !isMaxTime) {
                            onLongPressListener.onRecordFinishedListener();
                        }
                    }
                }
                break;
    ...
    private void startProgressAnimation() {
        ...
        mProgressAni.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                //录制动画结束时，即为录制全部完成
                if (onLongPressListener != null && isPressed) {
                    isPressed = false;
                    isMaxTime = true;
                    onLongPressListener.onRecordFinishedListener();
